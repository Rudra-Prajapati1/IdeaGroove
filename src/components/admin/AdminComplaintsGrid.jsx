...
import { ClipboardList, ChevronLeft, ChevronRight, MessageCircle } from "lucide-react";

const AdminComplaintsGrid = ({ complaints, searchTerm, filterType, filterStatus, onStatusRequest }) => {
  ...
  const filteredComplaints = useMemo(() => {
    return complaints.filter((item) => {
      ...
      const matchesType = filterType === "all" || item.complaintType === filterType;
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden font-inter mx-4 mb-10">
      <div className="p-6 flex items-center justify-between bg-[#062D1C]">
        <h3 className="text-lg font-bold text-white uppercase tracking-wider">Complaint Logs</h3>
        <span className="text-[10px] font-black text-white/50 bg-white/10 px-3 py-1.5 rounded-full uppercase">
          ...
          <tbody className="divide-y divide-gray-50">
            {currentData.map((item) => (
              <React.Fragment key={item.Complaint_ID}>
                <tr 
                  onClick={() => setExpandedId(expandedId === item.Complaint_ID ? null : item.Complaint_ID)}
                ...
                >
                  <td className="px-8 py-5 text-sm font-bold text-[#1B431C] flex items-center gap-2">
                    ...
                    <MessageCircle size={14} className="text-gray-300 group-hover:text-[#1B431C]" />
                  </td>
                  <td className="px-8 py-5 text-sm font-medium text-gray-700">{item.Student_Name}</td>
                  {/* TYPE COLUMN */}
                  ...
                  <td className="px-8 py-5 text-sm text-gray-400">{new Date(item.Date).toLocaleDateString()}</td>
                  ...
                  <tr className="bg-gray-50/50 animate-in fade-in slide-in-from-top-1 duration-200">
                    <td colSpan="6" className="px-8 py-4">
                      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                        <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Detailed Issue</h4>
      ...
      <div className="p-6 bg-gray-50/30 border-t border-gray-100 flex justify-between items-center">
        <p className="text-xs text-gray-400 font-medium">
          Showing <span className="font-bold text-gray-600">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-bold text-gray-600">{Math.min(currentPage * itemsPerPage, filteredComplaints.length)}</span> <br/> of <span className="font-bold text-gray-600">{filteredComplaints.length}</span> complaints
        </p>
