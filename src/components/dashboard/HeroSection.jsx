...
const HeroSection = ({ user, isPublic = false }) => {
  ...
  return (
    <section className="relative bg-linear-to-br from-[#1B431C] via-[#235324] to-[#153416] min-h-[500px] flex flex-col justify-center overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto w-full px-8 sm:px-16 z-10 pt-16 pb-24">
        {isPublic ? (
          ...
          <div className="flex flex-col md:flex-row items-start gap-10 md:gap-14">
            ...
            <div className="flex-1 text-white min-w-0">
              {/* Badge */}
              <span className="inline-block px-4 py-1.5 rounded-full bg-white/10 text-green-200 text-sm font-medium mb-4 backdrop-blur-sm border border-white/10">
                Student Profile
              </span>

              {/* Name */}
              <h1 className="capitalize text-4xl lg:text-6xl font-extrabold mb-2 leading-tight tracking-tight">
                <span className="text-3xl lg:text-4xl font-light">Meet, </span>
                <span className="text-green-300">{displayName}</span>
              </h1>

              {/* Username + Message Button (inline) */}
              {(username || showMessageButton) && (
                <div className="flex flex-wrap items-center gap-3 mb-6">
                  {username && (
                    <p className="text-base font-medium tracking-wide text-green-100/80">
                      {username}
                    </p>
                  )}
                  {showMessageButton && (
              ...
              {/* Info Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 animate-in fade-in slide-in-from-left-4 duration-700">
                {/* Email — spans 2 cols on xl so it doesn't truncate */}
                <div className={`${infoCardClass} sm:col-span-2`}>
                  <Mail className="text-green-300 w-4 h-4 shrink-0" />
                ...
                <div className={infoCardClass}>
                  <School className="text-green-300 w-4 h-4 shrink-0" />
                ...
                <div className={infoCardClass}>
                  <GraduationCap className="text-green-300 w-4 h-4 shrink-0" />
                  <span className="text-sm opacity-90 line-clamp-1">
                    {degreeName}
                  </span>
                </div>
                ...
                <div className={infoCardClass}>
                  <Calendar className="text-green-300 w-4 h-4 shrink-0" />
                  <span className="text-sm opacity-90">
                    Batch of {batchLabel}
                  </span>
                </div>

                {/* Hobbies — always full width */}
                {hobbies.length > 0 && (
                  <div
                    className={`${infoCardClass} items-start col-span-1 sm:col-span-2 xl:col-span-3`}
                  >
                    <Sparkles className="text-green-300 w-4 h-4 mt-0.5 shrink-0" />
            ...
            <div className="relative group shrink-0 self-center">
              <div className="absolute -inset-1 bg-linear-to-r from-green-400 to-emerald-600 rounded-full blur opacity-25 group-hover:opacity-50 transition duration-1000" />
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-12">
            <div className="text-white flex-1 text-center md:text-left">
              ...
              <h1 className="capitalize text-5xl lg:text-7xl font-extrabold mb-6 leading-tight tracking-tight">
                <span className="text-5xl font-light">Hi, </span>
                <span className="text-green-300">{displayName}</span>
              </h1>
