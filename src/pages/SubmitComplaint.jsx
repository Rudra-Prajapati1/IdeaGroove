...
const SubmitComplaint = () => {
  ...
  const topics = {
    Notes: ['Java by Khushal', 'C++ By Rudra', 'Statistics by Sejal'],
    Any_User: ['Khushal', 'Rudra', 'Sejal', 'Jinesh'],
    Groups: ['Dance club', 'Chess club', 'Music lovers', 'Skating']
  };
  ...
  const getStatusStyles = (status) => {
    switch (status) {
      case 'RESOLVED': return 'bg-[#1A3C20] text-white border-transparent';
      case 'IN PROGRESS': return 'bg-orange-50 text-orange-700 border-orange-100';
      default: return 'bg-white text-[#1A3C20] border-[#1A3C20]/20';
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FFFBEB] font-poppins pb-20">
      ...
      <div className="max-w-6xl mx-auto -mt-30 relative z-30 px-4">
        <h1 className="text-5xl font-extrabold mb-4 text-[#FFFBEB]">Submit a Complaint</h1>
        <div className="bg-[#1A3C20]/60 backdrop-blur-md border border-white/20 rounded-lg px-4 py-3 inline-block">
          <p className="text-sm text-[#FFFBEB]">Help us improve your student experience. We take every report seriously.</p>
        </div>
      </div>
        ...
        <div className="col-span-8 bg-white rounded-2xl p-8 border border-gray-100 shadow-sm">
          <div className="space-y-8">
            <div>
              ...
              <p className="text-xs text-gray-400 mb-4">Select the department most relevant to your issue.</p>
              ...
              <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {topics[formData.category].map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                  <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={18} />
                </div>
              </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mb-4">Provide as much detail as possible, including dates and locations.</p>
              <textarea 
                rows={6}
            ...
            <div className="flex items-start gap-3">
              ...
              />
              <label htmlFor="anon" className="text-xs text-gray-500 leading-relaxed">
                Submit this report anonymously. <span className="font-bold">Note:</span> We may not be able to follow up directly with you if this is checked.
              </label>
            </div>
        ...
        <div className="col-span-4">
          <div className="bg-[#f0f4f1] rounded-2xl p-6 border border-[#e0e7e1]">
            ...
            <ul className="space-y-5">
              {[
                { title: 'Be specific', desc: 'Include names, dates, and times where possible.' },
                { title: 'Stay Objective', desc: 'Describe the facts of the situation clearly.' },
                { title: 'Visuals Help', desc: 'Screenshots are the best way to prove digital errors.' },
                { title: 'Respect', desc: 'Our team is here to help; please maintain a professional tone.' }
              ].map((tip, idx) => (
                <li key={idx} className="flex gap-3">
                  <CheckCircle2 className="text-[#1A3C20] shrink-0 mt-0.5" size={16} />
                  <div>
                    <p className="text-sm font-bold text-[#1A3C20] mb-1">{tip.title}:</p>
      ...
      <section className="max-w-6xl mx-auto px-8 mt-12">
        <div className="mb-8">
          <h2 className="text-4xl font-extrabold text-[#1A3C20] mb-2">Your Reported Issues</h2>
          <p className="text-gray-500 text-sm">Track the progress of your submissions and view admin feedback.</p>
        </div>
              ...
              <div key={item.id} className="group transition-colors hover:bg-gray-50/30">
                ...
                >
                  ...
                  <div className="col-span-5 pr-4">
                    <h3 className="font-bold text-gray-800 text-sm">{item.subject}</h3>
                    <p className="text-[11px] text-gray-400 mt-0.5">Ref ID: #CMP-{item.id}</p>
                  </div>
                  <div className="col-span-2 text-[13px] text-gray-500 font-medium">
                    {item.date}
                  </div>
                  <div className="col-span-2">
                    <span className={`px-4 py-1.5 rounded-lg border text-[10px] font-black tracking-widest uppercase ${getStatusStyles(item.status)}`}>
                      {item.status}
                    </span>
                  </div>
                  <div className="col-span-1 flex justify-end">
                    {expandedId === item.id ? (
                      <ChevronUp className="text-gray-300 group-hover:text-[#1A3C20]" size={20} />
                    ) : (
                      <ChevronDown className="text-gray-300 group-hover:text-[#1A3C20]" size={20} />
                    )}
                  </div>
                </div>
                    ...
                    <div className="bg-white border border-[#1A3C20]/5 rounded-2xl p-6 flex gap-5 ml-[16.6%] max-w-2xl shadow-sm ring-1 ring-black/5">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                           <span className="text-[10px] font-black text-[#1A3C20] tracking-widest uppercase">Admin Update</span>
                           <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">({item.adminUpdate.date})</span>
                        </div>
