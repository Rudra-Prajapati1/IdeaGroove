...
const initialNotes = [
  { id: 1, title: "Operating Systems Notes", subject: "OS", description: "Complete notes covering process, memory, and scheduling.", uploadedBy: "Rohan Sharma", status: "active" },
  { id: 2, title: "DBMS Normalization", subject: "DBMS", description: "Detailed explanation of 1NF, 2NF, 3NF with examples.", uploadedBy: "Aisha Khan", status: "blocked" },
  { id: 3, title: "Computer Networks Cheatsheet", subject: "CN", description: "Quick revision notes for TCP/IP and OSI layers.", uploadedBy: "Kunal Verma", status: "active" },
  { id: 4, title: "React Hooks Summary", subject: "Web Dev", description: "Short notes on useState, useEffect, useContext.", uploadedBy: "Neha Patel", status: "active" },
  { id: 5, title: "Machine Learning Basics", subject: "AI", description: "Introductory ML concepts and algorithms.", uploadedBy: "Arjun Mehta", status: "blocked" },
];

export const notesStats = [
  { title: "Total Notes Uploaded", value: "12,450", infoText: "+12% this month", color: "green", type: "total" },
  { title: "Pending Review", value: "84", infoText: "Priority items", color: "yellow", type: "pending" },
  { title: "Blocked Content", value: "312", infoText: "Compliance removal", color: "red", type: "blocked" },
];
...
const AdminNotes = () => {
  ...
  const subjectOptions = ["OS", "DBMS", "CN", "Web Dev", "AI", "Physics", "Math"];
  ...
  const handleActionSubmit = async (e) => {
    ...
    setTimeout(() => {
      setNotes((prev) => 
        prev.map((n) => n.id === targetId ? { ...n, status: selectedAction === "block" ? "blocked" : "active" } : n)
      );
          ...
          <div className="bg-white w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl animate-in zoom-in-95 border border-gray-100 font-inter">
            ...
            <div className={`p-6 text-white flex justify-between items-center ${selectedAction === 'block' ? 'bg-red-600' : 'bg-[#1B431C]'}`}>
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold font-poppins capitalize">{selectedAction} Content</h2>
              </div>
            ...
            <form onSubmit={handleActionSubmit} className="p-6 flex flex-col gap-5">
              <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100 flex gap-3 text-xs text-blue-800">
                ...
                <p>Inform the uploader why the status changed. This will be sent as a direct notification.</p>
              </div>
              <textarea 
                rows="4"
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setModalOpen(false)} className="flex-1 py-3 font-bold text-gray-400 hover:bg-gray-50 rounded-xl">Cancel</button>
